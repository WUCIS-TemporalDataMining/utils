...
public class MirrorUCRDatasets {
    ...
    public static double[] padToPowerOfTwo(double[] input, String method) {
        // If input.length is 0, what are you even doing here?
        if (input.length == 0) {
            return input;
        }
        //Test if method is an acceptable value.
        List<String> list = Arrays.asList("zero","avg","last");
        if(!list.contains(method)) throw new IllegalArgumentException("Method must be: zero, avg, or last.");
        ...
        // if "zero" pad 0's; if "avg" pad mean of series, if "last" pad last value.
        switch (method) {
            case "zero":
                for (int i = input.length, j = input.length - 1; i < powerOfTwo; i++, j--) {
                    output[i] = 0.0;
